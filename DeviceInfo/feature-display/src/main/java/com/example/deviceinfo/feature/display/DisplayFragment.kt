...
class DisplayFragment : Fragment(), ShareableFragment {
    ...
    private fun loadData() {
        ...
        items.add(InfoItem("Screen Size", "${"%.2f".format(diagInch)} inches (diagonal)"))
        ...
        // Refresh rate — collect all supported modes for max rate
        val currentRate: Float
        val maxRate: Float
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            currentRate = ctx.display?.refreshRate ?: 60f
            val dm = ctx.getSystemService(Context.DISPLAY_SERVICE) as DisplayManager
            val disp = dm.getDisplay(android.view.Display.DEFAULT_DISPLAY)
            maxRate = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                disp.supportedModes.maxOfOrNull { it.refreshRate } ?: currentRate
            } else currentRate
        } else {
            @Suppress("DEPRECATION")
            currentRate = wm.defaultDisplay.refreshRate
            maxRate = currentRate
        }
        items.add(InfoItem("Refresh Rate", "${"%.1f".format(currentRate)} Hz", true))
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            items.add(InfoItem("HDR Support",
                if (hdrLabels.isEmpty()) "Not Supported" else hdrLabels.joinToString(", "), true))
        ...
        // Update refresh rate gauge
        val resStr  = "${widthPx}\u00d7${heightPx}"
        val dpiStr  = "$densityDpi dpi"
        val sizeStr = "${"%.2f".format(diagInch)}\""
        b.refreshRateGauge.update(currentRate, maxRate, resStr, dpiStr, sizeStr)
    }
