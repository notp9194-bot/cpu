        ...
        try {
            ...
            ids.forEachIndexed { index, id ->
                ...
                chars.get(CameraCharacteristics.LENS_INFO_AVAILABLE_APERTURES)
                    ...
                    ?.let { items.add(InfoItem("Aperture", it.joinToString(", ") { a -> "f/%.1f".format(a) })) }
                chars.get(CameraCharacteristics.LENS_INFO_AVAILABLE_FOCAL_LENGTHS)
                    ?.takeIf { it.isNotEmpty() }
                    ?.let { items.add(InfoItem("Focal Length", it.joinToString(", ") { f -> "%.1f mm".format(f) })) }
                chars.get(CameraCharacteristics.SENSOR_INFO_PHYSICAL_SIZE)
                    ?.let { items.add(InfoItem("Sensor Size", "%.2f x %.2f mm".format(it.width, it.height))) }
                ...
                items.add(InfoItem("Auto Focus", if (afModes != null && afModes.size > 1) "Yes" else "Fixed Focus"))
                items.add(InfoItem("Flash", if (chars.get(CameraCharacteristics.FLASH_INFO_AVAILABLE) == true) "Yes" else "No"))
