  ...
  object ExportUtils {
      ...
      fun exportToPdf(context: Context, tabName: String, data: Map<String, String>) {
          ...
          val paintTitle = Paint().apply {
              color = Color.parseColor("#006064"); textSize = 20f; isFakeBoldText = true; isAntiAlias = true
          }
          val paintSub = Paint().apply {
              color = Color.parseColor("#0097A7"); textSize = 11f; isFakeBoldText = true; isAntiAlias = true
          }
          val paintLabel = Paint().apply {
              color = Color.parseColor("#546E7A"); textSize = 12f; isAntiAlias = true
          }
          val paintValue = Paint().apply {
              color = Color.parseColor("#1A1A2E"); textSize = 12f; isFakeBoldText = true; isAntiAlias = true
          }
          val paintDivider = Paint().apply {
              color = Color.parseColor("#B2EBF2"); strokeWidth = 1f
          }
          ...
          val cv = canvas!!
          cv.drawText("CPU-A Device Info", margin, margin + 30f, paintTitle)
          cv.drawText("Tab: $tabName", margin, margin + 50f, paintSub)
          val dateStr = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault()).format(Date())
          cv.drawText("Generated: $dateStr", margin, margin + 68f, paintLabel)
          cv.drawLine(margin, margin + 74f, pageWidth - margin, margin + 74f, paintDivider)
